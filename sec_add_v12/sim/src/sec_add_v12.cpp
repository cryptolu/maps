...
/******************************************************************************
 *
 * interface to sec_add_v02 simulation
 *
 ******************************************************************************/
...
#define TARGET_A_BUF_ADDR 0x0300
#define TARGET_B_BUF_ADDR 0x0400

void load(Cpu *cpu)
{
	if (cpu->load("./sec_add_v12.bin") < 0)
	{
		printf("-- ERROR: can not load ./sec_add_v12.bin\n");
...
unsigned long int sec_add_v12_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint32_t a, uint32_t b, uint32_t *y)
{
    /* mask inputs */
    uint32_t a_buf[2];
    mask(rnd_gen_uint32, a, a_buf, a_buf + 1);
    cpu->copy_array_to_target(a_buf, 2, TARGET_A_BUF_ADDR);
	cpu->write_register(R0, TARGET_A_BUF_ADDR);

	uint32_t b_buf[4];
    mask(rnd_gen_uint32, b, b_buf, b_buf + 1);
	b_buf[2] = rnd_gen_uint32();
	b_buf[3] = rnd_gen_uint32();
...
void check_sec_algo(Options &options)
{
	...
	unsigned long int count = sec_add_v12_wrapper(rnd_gen_uint32, &cpu, a, b, &y);
...
void t_test_sec_algo(Options &options)
{
	...
	Progress_bar progress_bar(options.n_measure, std::cout, "Simulating sec_add_v12 ...\n");
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		a = a_fixed;
		b = b_fixed;
		sec_add_v12_wrapper(rnd_gen_uint32, &cpu, a, b, &y);
