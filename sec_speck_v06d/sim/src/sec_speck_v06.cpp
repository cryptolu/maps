...
/******************************************************************************
 *
 * interface to sec_speck_v06 simulation
 *
 ******************************************************************************/
...
void load(Cpu *cpu)
{
	if (cpu->load("./sec_speck_v06.bin") < 0)
	{
		printf("-- ERROR: can not load ./sec_speck_v06.bin\n");
...
unsigned long int sec_speck_v06_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint32_t *l, uint32_t *r, uint32_t *rk)
{
	/* mask inputs */
	uint32_t buffer[4];
...
void check_sec_algo(Options &options)
{
	...
	unsigned int long count = sec_speck_v06_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
...
void t_test_sec_algo(Options &options)
{
	...
	Progress_bar progress_bar(options.n_measure, std::cout, "Simulating sec_speck_v06 ...\n");
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		l = l_fixed;
		r = r_fixed;
		sec_speck_v06_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
