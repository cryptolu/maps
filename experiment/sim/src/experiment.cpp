...
unsigned long int experiment_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint32_t *l, uint32_t *r, uint32_t *rk)
{
	/* mask inputs */
	uint32_t buffer[4];
...
void check_sec_algo(Options &options)
{
	...
	unsigned int long count = experiment_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
...
{
	fprintf(stderr, "-- ERROR: t_test_sec_algo not coded yet!\n");
}
