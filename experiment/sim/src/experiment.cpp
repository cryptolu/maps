...
#define TARGET_BUFFER_ADDR 0x20000074
#define TARGET_RK_MASKED_ADDR 0x20000084
...
unsigned long int experiment_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint16_t rows[4], const uint32_t rk[26*2])
{
	/* mask inputs */
	uint16_t buffer[8];
...
void check_sec_algo(Options &options)
{
	...
	options.mem_size = 0x2000fe00;
	Cpu cpu(options);

	load(&cpu);
	cpu.reset();
	cpu.write_register(SP, 0x2000fb08); // align stack with verilog reference
	...
	const uint16_t expected_rows[] = {0xaa14, 0x9d08, 0x8da3, 0x3cb4};

	unsigned int long count = experiment_wrapper(rnd_gen_uint32, &cpu, rows, rk);
...
void t_test_sec_algo(Options &options)
{
	...
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		rows[2] = rows_fixed[2];
		rows[3] = rows_fixed[3];
		experiment_wrapper(rnd_gen_uint32, &cpu, rows, rk);
