...
#define TARGET_BUFFER_ADDR 0x0700
#define TARGET_RK_MASKED_ADDR 0x0800

void load(Cpu *cpu)
{
	if (cpu->load("./sec_speck_v13.bin") < 0)
	{
		printf("-- ERROR: can not load ./sec_speck_v13.bin\n");
...
unsigned long int sec_speck_v13_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint32_t *l, uint32_t *r, uint32_t *rk)
{
	/* mask inputs */
	uint32_t buffer[6];
...
void check_sec_algo(Options &options)
{
    ...
    uint32_t l = 0x8c6fa548;
    uint32_t r = 0x454e028b;
	uint32_t rk[] = {
	...
	unsigned int long count = sec_speck_v13_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
	printf("-- %lu instructions executed\n", count);
	printf("-- l = 0x%08x, r = 0x%08x => ", l, r);
	//if (l == 0x8c6fa548 && r == 0x454e028b)
    if (l == 0x3b726574 && r == 0x7475432d)
	{
		printf("ok\n");
	}
	else
...
void t_test_sec_algo(Options &options)
{
	...
	Progress_bar progress_bar(options.n_measure, std::cout, "Simulating sec_speck_v13 ...\n");
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		l = l_fixed;
		r = r_fixed;
		sec_speck_v13_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
