...
/******************************************************************************
 *
 * interface to sec_simon_v02 simulation
 *
 ******************************************************************************/
...
#define TARGET_BUFFER_ADDR 0x0300
#define TARGET_RK_MASKED_ADDR 0x0400

void load(Cpu *cpu)
{
	if (cpu->load("./sec_simon_v02.bin") < 0)
	{
		printf("-- ERROR: can not load ./sec_simon_v02.bin\n");
...
unsigned long int sec_simon_v02_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint32_t *l, uint32_t *r, uint32_t *rk)
{
	/* mask inputs */
	uint32_t buffer[5];
	...
	cpu->copy_array_to_target(buffer, 5, TARGET_BUFFER_ADDR);
...
//	uint32_t r = 0x20646e75;
    uint32_t l = 0x44c8fc20;
    uint32_t r = 0xb9dfa07a;
	uint32_t rk[] = {
	...
	unsigned int long count = sec_simon_v02_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
	printf("-- %lu instructions executed\n", count);
	printf("-- l = 0x%08x, r = 0x%08x => ", l, r);
//	if (l == 0x44c8fc20 && r == 0xb9dfa07a)
	if (l == 0x656b696c && r == 0x20646e75)
	{
		printf("ok\n");
	}
	else
...
//	const uint32_t r_fixed = 0x20646e75;
    const uint32_t l_fixed = 0x44c8fc20;
    const uint32_t r_fixed = 0xb9dfa07a;
	uint32_t rk[] = {
		0x03020100, 0x0b0a0908, 0x13121110, 0x1b1a1918,
		0x70a011c3, 0xb770ec49, 0x57e3e835, 0xd397bc42,
		0x94dcf81f, 0xbf4b5f18, 0x8e5dabb9, 0xdbf4a863,
	...
	Progress_bar progress_bar(options.n_measure, std::cout, "Simulating sec_simon_v02 ...\n");
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		l = l_fixed;
		r = r_fixed;
		sec_simon_v02_wrapper(rnd_gen_uint32, &cpu, &l, &r, rk);
