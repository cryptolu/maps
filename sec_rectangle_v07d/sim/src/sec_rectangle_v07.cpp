...
/******************************************************************************
 *
 * interface to sec_rectangle_v07 simulation
 *
 ******************************************************************************/
...
void load(Cpu *cpu)
{
	if (cpu->load("./sec_rectangle_v07.bin") < 0)
	{
		printf("-- ERROR: can not load ./sec_rectangle_v07.bin\n");
...
unsigned long int sec_rectangle_v07_wrapper(std::mt19937 &rnd_gen_uint32, Cpu *cpu, uint16_t rows[4], const uint32_t rk[26*2])
{
	/* mask inputs */
	uint16_t buffer[10];
...
void check_sec_algo(Options &options)
{
	...
	uint16_t rows[] = {0x3ee8, 0xeeef, 0x154a, 0x467a};
	const uint32_t rk[] =
	{
	...
	const uint16_t expected_rows[] = {0xffff, 0xffff, 0xffff, 0xffff};

	unsigned int long count = sec_rectangle_v07_wrapper(rnd_gen_uint32, &cpu, rows, rk);
...
void t_test_sec_algo(Options &options)
{
    ...
    uint16_t rows_fixed[] = {0x3ee8, 0xeeef, 0x154a, 0x467a};
	const uint32_t rk[] =
	{
		0xffffffff, 0xffffffff, 0x00ff01ff, 0x00ffff00,
		0x01000301, 0xfefffeff, 0x02fff802, 0x02010100,
	...
	Progress_bar progress_bar(options.n_measure, std::cout, "Simulating sec_rectangle_v07 ...\n");
	for (unsigned long int measure_idx = 0; measure_idx < options.n_measure; ++measure_idx)
	{
		...
		rows[2] = rows_fixed[2];
		rows[3] = rows_fixed[3];
		sec_rectangle_v07_wrapper(rnd_gen_uint32, &cpu, rows, rk);
